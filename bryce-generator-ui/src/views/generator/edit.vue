<template>
  <el-drawer v-model="visible" :size="1200" :with-header="false">
      ...
      <el-tab-pane label="属性设置" name="field">
        ...
        >
          ...
          <vxe-column field="primaryKey" title="主键">
            <template #default="{row}">
              <vxe-checkbox v-model="row.primaryKey"/>
            </template>
          </vxe-column>
        </vxe-table>
      </el-tab-pane>
      <el-tab-pane label="表单配置" name="form">
        ...
        >
          ...
          <vxe-column field="formItem" title="表单显示">
            <template #default="{row}">
              <vxe-checkbox v-model="row.formItem"/>
            </template>
          </vxe-column>
          <vxe-column field="formRequired" title="表单必填">
            <template #default="{row}">
              <vxe-checkbox v-model="row.formRequired"/>
            </template>
          </vxe-column>
          <vxe-column field="formValidator" title="表单校验" :edit-render="{ name: 'input' }"></vxe-column>
          <vxe-column field="formItemType" title="表单类型">
            <template #default="{row}">
              <vxe-select v-model="row.formItemType">
      ...
      <el-tab-pane label="列表配置" name="grid">
        ...
        >
          ...
          <vxe-column field="gridItem" title="列表显示">
            <template #default="{row}">
              <vxe-checkbox v-model="row.gridItem"/>
            </template>
          </vxe-column>
          <vxe-column field="gridSort" title="列表排序">
            <template #default="{row}">
              <vxe-checkbox v-model="row.gridSort"/>
            </template>
          </vxe-column>
        </vxe-table>
      </el-tab-pane>
      <el-tab-pane label="查询配置" name="query">
        ...
        >
          ...
          <vxe-column field="queryItem" title="查询显示">
            <template #default="{row}">
              <vxe-checkbox v-model="row.queryItem"/>
            </template>
          </vxe-column>
...
import {getById, updateFieldList} from '@/api/table'
import {list as fieldTypeList} from '@/api/fieldType'
...
const rowDrop = () => {
  nextTick(() => {
    const el: any = document.querySelector('.body--wrapper>.vxe-table--body tbody')
...
const getFieldTypeList = async () => {
  ...
  const {data} = await fieldTypeList()
  // 设置属性类型值
